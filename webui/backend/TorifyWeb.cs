...
class Server
{
    ...
    static void Main(string[] args)
    {
        ...
        // tray icon (sair pelo tray tambem)
        tray = new NotifyIcon();
        ...
        Application.Run(); // mantem vivo (winexe + tray)
    }
    ...
    static string Q_RealIp(){ return Z_Q("https://api.ipify.org", null, 0, "http") ?? "failed"; }
    static string Q_TorIp(){ return Z_Q("https://api.ipify.org", "127.0.0.1", socksPort, "socks5") ?? "offline"; }
    ...
    static void Handle(HttpListenerContext ctx)
    {
        ...
        // estaticos embutidos (nao depende de pasta)
        ...
        if (path == "/ip")
        {
            Z_Json(ctx, "{\"ok\":true,\"real\":\"" + (Q_RealIp() ?? "failed") + "\",\"tor\":\"" + (Q_TorIp() ?? "offline") + "\"}"); return;
        }
        if (path == "/start" && method == "POST")
        {
            string r = Q_StartTor();
            Z_Json(ctx, "{\"ok\":true,\"msg\":\"" + r + "\"}"); return;
        }
